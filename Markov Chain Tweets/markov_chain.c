...
Node* get_node_from_database(MarkovChain *markov_chain, char *data_ptr) {
    ...
    Node * word=markov_chain->database->first;
    while  (word!=NULL) {
        if (strcmp(data_ptr, word->data->data)==0){
            return word;
            }
        word = word->next;
    }
...
Node* add_to_database(MarkovChain *markov_chain, char *data_ptr) {
    ...
    char * new_data = malloc(strlen(data_ptr)+1);
    if (new_data == NULL) {
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain) {
    ...
    while(1) { //runs until a legal word is returned
        ...
        if (node->data->data[strlen(node->data->data)-1]!='.') { //validate the random word isn't a EOS(end of sentence) word(ends with '.')
            return node->data;
        }
    }
}
...
void generate_tweet(MarkovNode *first_node, int max_length) { // generates tweet
    if (first_node == NULL) { //validate input
        return;
    }
    int counter=0;
    MarkovNode *node = first_node;
    while (counter<max_length) { // go until the word counter reaches maximum words or on EOS word is reached
        if (node==NULL) {
            break;
        }
        if (node->data[strlen(node->data)-1]=='.') { //check if word ends with '.'
            printf("%s", node->data);
            break;
        }
        printf("%s ", node->data); //print word
        counter++;
